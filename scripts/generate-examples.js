...
function generateExamples(context) {
    ...
    for (const jsonFile of jsonFiles) {
        const parsedJson = JSON.parse(fs.readFileSync(path.join(SEARCH_DIR, context, jsonFile), 'utf-8'))
        const componentFolder = path.join(EXAMPLES_DIR, context, jsonFile.replace(".json", ""))
